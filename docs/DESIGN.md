...
The analytics engine is designed to derive high-level retail KPIs from raw spatial-temporal events:
- **Conversion Funnel**: Computed by intersecting unique `visitor_id`s across `ENTRY`, `BILLING_QUEUE_JOIN`, and `PURCHASE` events.
- **Heatmaps**: Generated by aggregating `dwell_ms` for `ZONE_DWELL` events across distinct `zone_id`s, normalized to highlight high-traffic areas.
- **Anomaly Detection**: Uses rolling statistical windows (e.g., Z-score on queue depth) to flag operational deviations like sudden checkout abandonment.
